<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <a href="{{ route('admin.events.create') }}" class="inline-flex items-center justify-center px-6 py-2.5 bg-teal-500 hover:bg-teal-600 active:bg-teal-700 font-bold text-sm text-white rounded-full shadow-md hover:shadow-lg hover:scale-102 active:scale-98 transform transition duration-150 cursor-pointer">
                ➕ Tambah Acara Baru
            </a>
        </div>
    </x-slot>
                                        ...
                                        <tr class="hover:bg-slate-50/50 transition duration-150">
                                            ...
                                            <td class="py-4 px-4">
                                                <div class="flex items-center justify-center gap-2">
                                                    <!-- View -->
                                                    <a href="{{ route('admin.events.show', $event) }}" class="inline-flex items-center justify-center p-2 bg-slate-100 text-slate-600 hover:bg-slate-200 active:bg-slate-300 rounded-xl transition duration-150" title="Detail">
                                                        👁️
                                                    </a>
                                                    <!-- Edit -->
                                                    <a href="{{ route('admin.events.edit', $event) }}" class="inline-flex items-center justify-center p-2 bg-cyan-50 text-cyan-600 hover:bg-cyan-100 active:bg-cyan-200 rounded-xl transition duration-150" title="Edit">
                                                        ✏️
                                                    </a>
                                                    <!-- Delete -->
                                                    <form action="{{ route('admin.events.destroy', $event) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus acara ini?');" class="inline">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="inline-flex items-center justify-center p-2 bg-rose-50 text-rose-600 hover:bg-rose-100 active:bg-rose-200 rounded-xl transition duration-150 cursor-pointer" title="Hapus">
                                                            🗑️
                                                        </button>
                                                    </form>
                                                </div>
