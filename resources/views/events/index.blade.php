            ...
            @else
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach ($events as $event)
                        <div class="bg-white/80 backdrop-blur-md overflow-hidden border border-slate-100 shadow-lg hover:shadow-xl hover:scale-102 transform transition duration-300 rounded-3xl flex flex-col justify-between p-6">
                            <div>
                                <!-- Date tag -->
                                <span class="inline-flex items-center gap-1.5 bg-emerald-50 text-emerald-700 border border-emerald-100 text-xs font-bold px-3 py-1.5 rounded-full mb-4 shadow-sm">
